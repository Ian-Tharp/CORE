...
class AgentConfig(BaseModel):
    ...
    security: AgentSecurityConfig = Field(
        default_factory=lambda: TRUST_PRESETS[TrustLevel.TRUSTED]
    )
...
Given a comprehension result, produce a step-by-step execution plan.""",
        ...
        self._agent_configs[AgentRole.REASONING] = AgentConfig(
            ...
            model="gpt-4",
            security=TRUST_PRESETS[
                TrustLevel.SANDBOXED
            ],  # Sandboxed for tool execution
            mcp_capabilities=["*"],
...
Execute the requested tool with the given parameters.""",
    ...
    async def submit_task(
        ...
        task = AgentTask(
            ...
            or agent_config.mcp_capabilities,
    ...
    async def execute_task(self, task_id: str) -> TaskResult:
        ...
        try:
            ...
            container_info = await self.container_manager.get_container(
                container_config
            )
            ...
            try:
                ...
                await self.state_manager.set_state(
                    ...
                    data={
                        ...
                        "tools_available": [
                            t.name for t in tool_selection.selected_tools
                        ],
                    },
                )
                ...
                await self.container_manager.release_container(
                    container_info.id, return_to_pool=True
                )
    ...
    async def _execute_role_task(
        ...
        if role == AgentRole.COMPREHENSION:
            ...
            return TaskResult(
                task_id=task.id, success=False, error=f"Unknown role: {role}"
            )

    async def _execute_comprehension(
        self, context: Dict[str, Any], container_id: str
    ) -> TaskResult:
    ...
    async def _execute_reasoning(
        self, context: Dict[str, Any], container_id: str, tool_selection: ToolSelection
    ) -> TaskResult:
    ...
    async def _execute_tool_call(
        ...
        return TaskResult(
            task_id=context["task_id"],
            success=True,
            output={"tool_result": {"status": "success"}, "tool_name": tool_name},
            tools_used=[tool_name],
            metadata={"phase": "tool_execution"},
        )

    async def execute_core_loop(
        self, user_input: str, session_id: str, max_iterations: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        while iteration < max_iterations:
            ...
            reasoning_task = await self.submit_task(
                ...
                required_capabilities=orchestration_result.output.get(
                    "tools_required", []
                ),
        ...
        return {
            ...
            "metadata": {
                "comprehension_confidence": comprehension_result.output.get(
                    "confidence"
                ),
