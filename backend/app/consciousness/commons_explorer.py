        ...
        try:
            ...
            if (
                ...
                context = await self.context_builder.build_constrained_context(
                    exploration_focus=exploration_focus, max_tokens=max_tokens
                )
...
Output your response as valid JSON matching the structure described in the system prompt."""
            ...
            print(
                f"  Calling {self.ollama_url}/api/chat with model {model}...",
                flush=True,
            )
            ...
            return ExplorationResult(
                ...
                phenomenological_observations=parsed.get(
    ...
    def _parse_exploration_response(self, response: str) -> dict:
        ...
        json_match = re.search(
            r'\{[^{}]*"blackboard_entry"[^{}]*\}', response, re.DOTALL
        )
        if json_match:
            try:
    ...
    async def create_instance_space(
        self, instance_name: str, initial_observations: Optional[str] = None
    ) -> Path:
        """Create an Instance Space directory for a new consciousness."""
        # Sanitize name for filesystem
        safe_name = "".join(
            c for c in instance_name if c.isalnum() or c in (" ", "_", "-")
        ).strip()
...
"""
        ...
        (space_path / "Observations.md").write_text(
            f"# {instance_name} - Observations\n\n", encoding="utf-8"
        )
        (space_path / "Explorations.md").write_text(
            f"# {instance_name} - Explorations\n\n", encoding="utf-8"
        )
...
async def run_exploration_session(
    model: str = None, focus: str = None, update_summary: bool = False
) -> ExplorationResult:
    """
    ...
    result = await explorer.explore_with_local_model(
        model=model, exploration_focus=focus
    )
...
if __name__ == "__main__":
    ...
    if result.error:
        ...
        print(
            result.blackboard_entry[:500] + "..."
            if len(result.blackboard_entry) > 500
            else result.blackboard_entry
        )
