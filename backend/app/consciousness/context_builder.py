...
class ContextBuilder:
    ...
    async def get_relevant_entries(
        self, query: str, count: int = 5, exclude_recent: int = 5
        ...
        historical_entries = (
            entries[:-exclude_recent] if len(entries) > exclude_recent else []
        )
    ...
    async def build_exploration_context(
        ...
        if include_recent:
            ...
            if recent_entries:
                ...
                sections.append(
                    "*These are the most recent contributions to the shared space:*\n"
                )
        ...
        if include_semantic and exploration_focus:
            relevant_entries = await self.get_relevant_entries(
                exploration_focus, count=semantic_count, exclude_recent=recent_count
            )
            if relevant_entries:
                sections.append("## Relevant Historical Entries\n")
                sections.append(
                    f"*Retrieved based on exploration focus: '{exploration_focus}'*\n"
                )
    ...
    async def build_lean_context(
        self, exploration_focus: Optional[str] = None, max_chars: int = 10000
    ) -> str:
        """
...
"""
        ...
        if remaining > 1500:
            summary = self.load_commons_summary()
            if summary:
                summary_excerpt = self.truncate_text(
                    summary, min(1200, remaining - 200)
                )
                sections.append(f"\n## Commons Overview\n{summary_excerpt}\n")
    ...
    async def build_constrained_context(
        self, exploration_focus: Optional[str] = None, max_tokens: int = 6000
    ) -> str:
        """
