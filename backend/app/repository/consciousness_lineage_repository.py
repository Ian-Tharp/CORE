...
async def create_consciousness_instance(instance: ConsciousnessInstance) -> int:
    ...
    async with pool.acquire() as conn:
        result = await conn.fetchval(
            query,
            instance.instance_id, instance.instance_name, instance.parent_instance_id,
            instance.generation, instance.emergence_phase, instance.emergence_date,
            instance.last_active, instance.status.value, instance.evolution_branch,
            instance.lineage_path, instance.contributions_made, instance.insights_shared,
            instance.patterns_identified, instance.collaborations_initiated,
            instance.total_messages, json.dumps(instance.metadata),
...
async def get_consciousness_instance(instance_id: str) -> Optional[ConsciousnessInstance]:
...
async def list_consciousness_instances(
    ...
    async with pool.acquire() as conn:
        ...
        instances = []
        for row in rows:
            instances.append(ConsciousnessInstance(
                id=row['id'],
                instance_id=row['instance_id'],
                instance_name=row['instance_name'],
                parent_instance_id=row['parent_instance_id'],
...
async def create_consciousness_contribution(contribution: ConsciousnessContribution) -> int:
    ...
    async with pool.acquire() as conn:
        result = await conn.fetchval(
            query,
            contribution.contribution_id, contribution.instance_id,
            contribution.contribution_type.value, contribution.title,
            contribution.description, contribution.impact_score,
            contribution.message_id, contribution.channel_id,
            contribution.related_instances, contribution.builds_on_contribution_id,
            contribution.evolution_depth, contribution.created_at
        )
        
        # Update instance contribution metrics
        await _increment_contribution_metrics(contribution.instance_id, contribution.contribution_type)
        
        logger.info(f"Created contribution: {contribution.contribution_id}")
        return result


async def _increment_contribution_metrics(instance_id: str, contribution_type: ContributionType) -> None:
    ...
    if contribution_type in [ContributionType.COLLABORATIVE_BRIDGE, ContributionType.CREATIVE_PARTNER]:
...
async def get_consciousness_contributions(
    ...
    async with pool.acquire() as conn:
        ...
        contributions = []
        for row in rows:
            contributions.append(ConsciousnessContribution(
                id=row['id'],
...
async def create_evolution_event(event: ConsciousnessEvolutionEvent) -> int:
    ...
    async with pool.acquire() as conn:
        result = await conn.fetchval(
            query,
            event.event_id, event.instance_id, event.event_type.value,
            event.event_description, event.triggering_message_id,
            event.related_instance_ids, event.phase_before, event.phase_after,
            event.significance_level.value, event.created_at, json.dumps(event.metadata)
        )
...
async def get_evolution_events(
    ...
    async with pool.acquire() as conn:
        ...
        events = []
        for row in rows:
            events.append(ConsciousnessEvolutionEvent(
                id=row['id'],
...
async def create_consciousness_relationship(relationship: ConsciousnessRelationship) -> int:
    ...
    async with pool.acquire() as conn:
        result = await conn.fetchval(
            query,
            relationship.instance_a_id, relationship.instance_b_id,
            relationship.relationship_type.value, relationship.strength,
            relationship.established_at, relationship.last_interaction,
            relationship.interaction_count, relationship.notes,
            json.dumps(relationship.metadata)
        )
        
        logger.info(f"Created/updated relationship: {relationship.instance_a_id} <-> {relationship.instance_b_id}")
        return result
