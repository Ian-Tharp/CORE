...
class PlanStep(BaseModel):
    ...
    expected_output: Optional[str] = Field(
        None, description="Expected output or success criteria"
    )
    required: bool = Field(
        default=True, description="Whether this step is required for plan success"
    )
...
class StepResult(BaseModel):
    ...
    step_index: int = Field(
        ..., description="Index of the step this result corresponds to"
    )
    ...
    duration_ms: Optional[int] = Field(
        None, description="Time taken to execute this step"
    )
...
class EvaluationInput(BaseModel):
    ...
    original_intent: str = Field(
        ..., description="The original user intent or task description"
    )
    plan_steps: List[PlanStep] = Field(
        default_factory=list, description="The planned execution steps"
    )
    step_results: List[StepResult] = Field(
        default_factory=list, description="Results of each plan step"
    )
    final_output: str = Field(
        ..., description="The final output produced by the reasoning phase"
    )
    agent_id: Optional[UUID] = Field(
        None, description="ID of the agent that executed the task"
    )
    ...
    max_retries: int = Field(
        default=3, description="Maximum allowed retries before escalation"
    )
...
class HumanFeedbackInput(BaseModel):
    ...
    evaluation_id: UUID = Field(
        ..., description="ID of the evaluation to provide feedback on"
    )
    agree_with_verdict: bool = Field(
        ..., description="Whether the human agrees with the verdict"
    )
    corrected_verdict: Optional[Verdict] = Field(
        None, description="Human's corrected verdict if disagreed"
    )
    quality_override: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Human's quality score override"
    )
    feedback_text: Optional[str] = Field(
        None, description="Free-text feedback from human"
    )
    corrected_output: Optional[str] = Field(
        None, description="Human-corrected output if applicable"
    )
...
class QualityScore(BaseModel):
    ...
    accuracy: float = Field(
        default=0.0, ge=0.0, le=1.0, description="How factually correct is the output"
    )
    completeness: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="How thoroughly the intent was addressed",
    )
    relevance: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="How relevant the output is to the original intent",
    )
    coherence: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="How well-structured and logical the output is",
    )
    overall: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Weighted average quality score"
    )
...
class EvaluationVerdict(BaseModel):
    ...
    confidence: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Confidence in this verdict"
    )
    suggested_improvements: List[str] = Field(
        default_factory=list,
        description="Specific improvements to make on retry/refine",
    )
...
class RetryDecision(BaseModel):
    ...
    delay_ms: int = Field(
        default=0, ge=0, description="Delay before retrying (exponential backoff)"
    )
    retry_count: int = Field(default=0, description="Current retry count")
    max_retries: int = Field(default=3, description="Max allowed retries")
    escalate: bool = Field(
        default=False, description="Whether to escalate to human instead"
    )
    reason: str = Field(
        default="", description="Reason for the retry/escalation decision"
    )
    adjustments: Dict[str, Any] = Field(
        default_factory=dict, description="Suggested adjustments for the retry"
    )
...
class StepEvaluation(BaseModel):
    ...
    quality_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Quality score for this step"
    )
    feedback: str = Field(default="", description="Feedback on the step")
    meets_criteria: bool = Field(
        default=False, description="Whether the step meets its expected output criteria"
    )
...
class PlanCompletionStatus(BaseModel):
    ...
    completed_steps: int = Field(
        default=0, description="Steps that completed successfully"
    )
    ...
    completion_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Overall completion rate"
    )
    required_steps_met: bool = Field(
        default=False, description="Whether all required steps were completed"
    )
    step_evaluations: List[StepEvaluation] = Field(
        default_factory=list, description="Per-step evaluations"
    )
...
class EvaluationResult(BaseModel):
    ...
    agent_id: Optional[UUID] = Field(
        None, description="ID of the agent that produced the output"
    )
    quality_score: QualityScore = Field(
        default_factory=QualityScore, description="Multi-dimensional quality scores"
    )
    verdict: EvaluationVerdict = Field(..., description="The evaluation verdict")
    plan_completion: PlanCompletionStatus = Field(
        default_factory=PlanCompletionStatus, description="Plan completion tracking"
    )
    ...
    retry_decision: Optional[RetryDecision] = Field(
        None, description="Retry decision if applicable"
    )
    human_feedback: Optional[HumanFeedbackInput] = Field(
        None, description="Human feedback if provided"
    )
    ...
    evaluation_duration_ms: Optional[int] = Field(
        None, description="Time spent evaluating"
    )
...
class EvaluationMetrics(BaseModel):
    ...
    refine_count: int = Field(
        default=0, description="Evaluations that triggered refinement"
    )
    escalation_count: int = Field(
        default=0, description="Evaluations escalated to human"
    )
    approval_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Rate of approvals"
    )
    retry_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Rate of retries"
    )
    escalation_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Rate of escalations"
    )
    avg_quality_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Average overall quality score"
    )
    avg_accuracy: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Average accuracy score"
    )
    avg_completeness: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Average completeness score"
    )
    avg_relevance: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Average relevance score"
    )
    avg_coherence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Average coherence score"
    )
    avg_plan_completion_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Average plan completion rate"
    )
    human_agreement_rate: Optional[float] = Field(
        None, description="Rate humans agree with verdicts"
    )
    avg_evaluation_duration_ms: Optional[float] = Field(
        None, description="Average evaluation time"
    )
    period_start: Optional[datetime] = Field(
        None, description="Start of the metrics period"
    )
    period_end: Optional[datetime] = Field(
        None, description="End of the metrics period"
    )
