...
class AgentActivityEvent(BaseEvent):
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional action-specific data"
    )
...
class CouncilEvent(BaseEvent):
    ...
    council_session_id: str = Field(
        ..., description="Unique identifier for the council session"
    )
    ...
    content: Optional[str] = Field(
        None, description="Content of the perspective/vote/synthesis"
    )
...
class NotificationEvent(BaseEvent):
    ...
    category: Optional[str] = Field(
        None, description="Notification category for grouping"
    )
    dismissible: bool = Field(
        default=True, description="Whether notification can be dismissed"
    )
    auto_dismiss_ms: Optional[int] = Field(
        None, ge=0, description="Auto-dismiss after milliseconds"
    )
