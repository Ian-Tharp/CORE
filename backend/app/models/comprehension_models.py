...
class ComprehensionInput(BaseModel):
    ...
    source_id: Optional[str] = Field(
        None, description="ID of the source (user_id, agent_id, tool_name)"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional context from the source"
    )
    conversation_id: Optional[str] = Field(
        None, description="Conversation context if applicable"
    )
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When the input was received"
    )
...
class ExtractedEntity(BaseModel):
    ...
    entity_type: str = Field(
        ..., description="Type of entity (person, topic, tool, etc.)"
    )
...
class IntentAnalysis(BaseModel):
    ...
    urgency: UrgencyLevel = Field(
        default=UrgencyLevel.MEDIUM, description="Assessed urgency level"
    )
    confidence: float = Field(
        ge=0.0, le=1.0, description="Overall confidence in intent parsing"
    )
    keywords: List[str] = Field(
        default_factory=list, description="Key terms extracted from input"
    )
    requires_clarification: bool = Field(
        default=False, description="Whether the input is ambiguous"
    )
...
class MemoryMatch(BaseModel):
    ...
    memory_tier: str = Field(
        ..., description="Which tier: semantic, episodic, procedural"
    )
    metadata: Dict[str, Any] = Field(
...
class ContextMatch(BaseModel):
    ...
    has_relevant_context: bool = Field(
        default=False, description="Whether any relevant context was found"
    )
    ...
    def total_matches(self) -> int:
        return (
            len(self.semantic_matches)
            + len(self.episodic_matches)
            + len(self.procedural_matches)
        )
...
class MatchedCapability(BaseModel):
    ...
    match_score: float = Field(
        ge=0.0, le=1.0, description="How well this capability matches"
    )
    source_type: str = Field(
        ..., description="Where this capability comes from: agent, tool, service"
    )
    source_id: str = Field(
        ..., description="ID of the agent/tool/service providing this capability"
    )
...
class CapabilityMatch(BaseModel):
    ...
    has_capable_agents: bool = Field(
        default=False, description="Whether any agent can handle this"
    )
    best_agent_id: Optional[str] = Field(
        None, description="ID of the best-matched agent"
    )
    best_agent_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Best agent match score"
    )
...
class ComplexityScore(BaseModel):
    ...
    overall: float = Field(
        ge=0.0,
        le=1.0,
        description="Overall complexity score (0=trivial, 1=extremely complex)",
    )
...
class ComprehensionResult(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique comprehension result ID"
    )

    # Original input
    input: ComprehensionInput = Field(
        ..., description="The original input that was analyzed"
    )

    # Analysis results
    intent: IntentAnalysis = Field(..., description="Parsed intent analysis")
    context: ContextMatch = Field(
        default_factory=ContextMatch, description="Memory context matches"
    )
    capabilities: CapabilityMatch = Field(
        default_factory=CapabilityMatch, description="Capability matches"
    )
    ...
    suggested_task_type: str = Field(
        default="general", description="Suggested task type for routing"
    )
    suggested_priority: int = Field(
        default=5, ge=1, le=10, description="Suggested priority (1-10)"
    )
    handling_mode: HandlingMode = Field(
        default=HandlingMode.SINGLE_AGENT,
        description="Whether single or multi-agent handling is needed",
    )
...
class ComprehensionFeedback(BaseModel):
    ...
    comprehension_id: UUID = Field(
        ..., description="ID of the comprehension result being rated"
    )
    score: float = Field(
...
class SystemCapability(BaseModel):
    ...
    provider_type: str = Field(
        ..., description="Type of provider: agent, tool, service"
    )
