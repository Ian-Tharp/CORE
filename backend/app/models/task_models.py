...
class Task(BaseModel):
    ...
    task_type: str = Field(
        ..., description="Type of task (research, code, analysis, monitoring, etc.)"
    )
    payload: Dict[str, Any] = Field(
        default_factory=dict, description="Task-specific data and parameters"
    )
    priority: int = Field(
        default=5, ge=1, le=10, description="Task priority (1-10, higher = more urgent)"
    )
    ...
    preferred_model: Optional[str] = Field(
        None,
        description="Preferred LLM model (e.g., 'ollama/llama3.2' or 'claude-sonnet')",
    )
    ...
    human_override: bool = Field(
        default=False, description="Was this assignment overridden by human?"
    )
    override_reason: Optional[str] = Field(
        None, description="Reason for human override"
    )
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Task creation timestamp"
    )
    assigned_at: Optional[datetime] = Field(
        None, description="Task assignment timestamp"
    )
    completed_at: Optional[datetime] = Field(
        None, description="Task completion timestamp"
    )
    duration_ms: Optional[int] = Field(
        None, description="Task execution duration in milliseconds"
    )
...
class TaskAssignment(BaseModel):
    ...
    agent_response: str = Field(
        ..., description="Agent's response (accept, refuse, suggest_alternative)"
    )
    refusal_reason: Optional[str] = Field(None, description="Reason for task refusal")
    suggested_agent: Optional[str] = Field(
        None, description="Agent suggested as alternative"
    )
    confidence_score: float = Field(
        ge=0.0, le=1.0, description="Agent's confidence in handling this task"
    )
...
class TaskResult(BaseModel):
    ...
    status: str = Field(
        ..., description="Final task status (completed, failed, partial)"
    )
...
class TaskRoutingScore(BaseModel):
    ...
    load_score: float = Field(
        ge=0.0, le=1.0, description="Current load score (lower load = higher score)"
    )
    trust_score: float = Field(
        ge=0.0, le=1.0, description="Trust score from historical performance"
    )
    model_preference_score: float = Field(
        ge=0.0, le=1.0, description="Model preference alignment score"
    )
    task_type_performance_score: float = Field(
        ge=0.0, le=1.0, description="Historical performance on this task type"
    )
