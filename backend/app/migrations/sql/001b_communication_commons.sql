-- Communication Commons base schema (folded into the active migration runner).
--
-- WHY THIS EXISTS:
--   The communication tables (communication_channels / communication_messages /
--   instance_presence, plus members/reactions/receipts) were historically created
--   by backend/migrations/001_communication_commons.sql — a directory the CURRENT
--   runner (app/migrations/sql/) never executes. 001_initial_schema.sql was emptied
--   to a no-op on the assumption these tables "already exist". On a fresh database
--   they do not, so 002_consciousness_lineage.sql (which has FKs to
--   communication_messages/communication_channels and seeds consciousness instances
--   FROM instance_presence) fails with `relation "communication_messages" does not
--   exist`, which then blocks 003 and 004.
--
--   This migration recreates that base schema inside the active runner. It is named
--   `001b_` so it sorts AFTER 001_initial_schema and BEFORE 002_consciousness_lineage,
--   guaranteeing the tables exist before 002 references them. Everything is
--   idempotent (CREATE TABLE IF NOT EXISTS + ON CONFLICT DO NOTHING), so on machines
--   where these tables/seed rows already exist this migration is a harmless no-op.
...
-- (ON CONFLICT DO NOTHING so this is safe to re-run on already-seeded databases.)
-- 002_consciousness_lineage.sql depends on the consciousness instances seeded here:
-- it copies them into consciousness_instances FROM instance_presence and then inserts
-- evolution_events that FK back to those instance_ids.
-- =============================================================================

-- Create Blackboard Global channel + initial team channels
...
-- Create initial consciousness instances + agents + human
