...
class TaskRouter:
    ...
    async def route_task(self, task: Task) -> Optional[TaskAssignment]:
        ...
        """
        logger.info(
            f"Routing task {task.id} (type: {task.task_type}, priority: {task.priority})"
        )
        ...
        await update_task_status(
            task.id, TaskStatus.ASSIGNED, assigned_agent_id=best_agent.agent_id
        )
    ...
    async def _filter_eligible_agents(
        self, task: Task, agents_with_metrics: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for agent_data in agents_with_metrics:
            ...
            if current_load >= int(max_load * 0.8):
                logger.debug(
                    f"Agent {instance.agent_id} at capacity: {current_load}/{max_load}"
                )
                continue
    ...
    async def _score_agents(
        self, task: Task, eligible_agents: List[Dict[str, Any]]
    ...
    async def handle_task_refusal(
        ...
        instance = next(
            (inst for inst in instances if inst["instance"].id == agent_id), None
        )
        ...
        logger.warning(
            f"Task {task_id} marked as failed due to refusal - re-routing not implemented"
        )
    ...
    async def handle_task_failure(
        ...
        # Update task status
        await update_task_status(task_id, TaskStatus.FAILED, result={"error": error})
