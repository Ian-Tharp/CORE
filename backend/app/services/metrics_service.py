...
class EmbeddingMetrics:
    ...
    def record_embedding_latency(
        ...
        metric_data = {
            ...
            "chars_per_sec": (
                total_chars / (latency_ms / 1000)
                if total_chars and latency_ms > 0
                else None
            ),
            "texts_per_sec": (
                text_count / (latency_ms / 1000) if latency_ms > 0 else None
            ),
        }
    ...
    def get_recent_metrics(
        self, hours: int = 24, operation: Optional[str] = None
    ) -> List[Dict]:
...
def record_embedding_performance(
    operation: str,
    model: str,
    text_count: int,
    latency_ms: float,
    ...
    _metrics.record_embedding_latency(
        operation, model, text_count, latency_ms, total_chars
    )


def get_embedding_metrics(
    hours: int = 24, operation: Optional[str] = None
) -> List[Dict]:
