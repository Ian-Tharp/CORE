...
class InstanceManager:
    ...
    async def spawn_instance(self, config: InstanceConfig) -> InstanceInfo:
        ...
        container_config = {
            ...
            "cpu_quota": int(
                config.cpu_limit * 100000
            ),  # Docker CPU quota in microseconds
            "cpu_period": 100000,
    ...
    async def _wait_for_healthy(self, container_id: str, timeout: int = 30) -> None:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                raise RuntimeError(
                    f"Container {container_id} disappeared during startup"
                )

        raise TimeoutError(
            f"Container {container_id} did not become healthy within {timeout}s"
        )
            ...
            for container in containers:
                try:
                    ...
                    instance_info = InstanceInfo(
                        ...
                        last_heartbeat=(
                            instance_record.last_heartbeat if instance_record else None
                        ),
                    )
                    ...
                    logger.warning(
                        f"Error processing container {container.id[:12]}: {e}"
                    )
                    continue
    ...
    async def get_instance_status(self, container_id: str) -> Optional[InstanceInfo]:
        ...
        try:
            ...
            return InstanceInfo(
                ...
                cpu_usage=cpu_usage,
                created_at=(
                    instance_record.created_at if instance_record else datetime.utcnow()
                ),
                last_heartbeat=(
                    instance_record.last_heartbeat if instance_record else None
                ),
    ...
    async def scale_instances(self, role: str, target_count: int) -> Dict[str, Any]:
        ...
        try:
            containers = self.docker_client.containers.list(
                all=True, filters={"label": f"core.role={role}"}
            )
        ...
        if current_count < target_count:
            ...
            for i in range(spawn_count):
                try:
                    ...
                    logger.error(
                        f"Failed to spawn instance {i+1}/{spawn_count} for role {role}: {e}"
                    )
                    actions_taken.append({"action": "spawn_failed", "error": str(e)})
            ...
            for instance in instances_to_stop:
                try:
                    ...
                    logger.error(
                        f"Failed to stop instance {instance['container_id']}: {e}"
                    )
                    actions_taken.append(
                        {
                            "action": "stop_failed",
        ...
        final_count = len(
            [c for c in final_containers if c.status in ["running", "restarting"]]
        )
