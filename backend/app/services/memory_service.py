...
class MemoryService:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"LangMem manager initialization failed (non-fatal): {langmem_err}"
                )
                self.langmem_manager = None
    ...
    async def store_knowledge(
        ...
        try:
            ...
            logger.info(
                f"Stored semantic memory: {memory_id} from agent {source_agent_id}"
            )
            return memory_id
    ...
    async def search_knowledge(
        ...
        try:
            ...
            logger.debug(
                f"Found {len(memories)} semantic memories for query: {query[:50]}"
            )
            return memories
    ...
    async def get_agent_experiences(
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {len(memories)} episodic memories for agent {agent_id}"
            )
            return memories
    ...
    async def store_procedure(
        ...
        try:
            ...
            logger.info(
                f"Stored procedure '{procedure_name}' for role {role}: {memory_id}"
            )
            return memory_id
    ...
    async def search_procedures(
        ...
        try:
            ...
            memories = await search_procedural_memories(
                role=role, query_embedding=query_embedding
            )
            ...
            logger.debug(
                f"Found {len(procedures)} procedures matching query: {query[:50]}"
            )
            return procedures
    ...
    async def update_procedure_outcome(self, procedure_id: UUID, success: bool) -> bool:
    ...
    async def get_relevant_context(
        ...
        try:
            ...
            total_items = (
                len(semantic_context) + len(episodic_context) + len(procedural_context)
            )
            ...
            logger.debug(
                f"Generated context with {total_items} items for query: {query[:50]}"
            )
            return context
    ...
    async def clear_agent_memories(
        self, agent_id: str, tier: Optional[str] = None
    ) -> Dict[str, int]:
        ...
        try:
            ...
            logger.info(
                f"Cleared {total_cleared} memories for agent {agent_id} (tier: {tier or 'all'})"
            )
