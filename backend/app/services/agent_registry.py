...
class TaskRefusal(BaseModel):
    ...
    suggested_agent: Optional[str] = Field(
        None, description="Suggested alternative agent"
    )
...
class AgentDeregistration(BaseModel):
    ...
    final_state: Dict[str, Any] = Field(
        default_factory=dict, description="Final agent state"
    )
...
class AgentRegistry:
    ...
    async def register_agent(
        self, registration_payload: AgentRegistrationPayload
    ) -> AgentConfig:
        """
        ...
        agent_id = (
            instance.agent_id or f"{registration_payload.role}-{str(uuid4())[:8]}"
        )
        ...
        return AgentConfig(
            ...
            memory_config={"max_context_length": 4000, "conversation_memory": True},
        )
    ...
    async def handle_heartbeat(
        self, agent_id: str, heartbeat_data: AgentHeartbeatData
    ) -> Dict[str, Any]:
        """
        ...
        logger.debug(
            f"Processed heartbeat from {agent_id} (status: {heartbeat_data.status})"
        )

        return response

    async def handle_task_completion(
        self, agent_id: str, completion: TaskCompletion
    ) -> None:
        """
        ...
        await increment_task_completed(
            agent_info["instance_id"], completion.duration_ms
        )
        ...
        logger.info(
            f"Agent {agent_id} completed task {completion.task_id} in {completion.duration_ms}ms"
        )
    ...
    async def handle_task_refusal(self, agent_id: str, refusal: TaskRefusal) -> None:
        ...
        logger.info(
            f"Agent {agent_id} refused task {refusal.task_id}: {refusal.reason}"
        )
        ...
        if refusal.suggested_agent:
            logger.info(
                f"Agent {agent_id} suggested reassigning to {refusal.suggested_agent}"
            )

    async def deregister_agent(
        self, agent_id: str, deregistration: AgentDeregistration
    ) -> None:
        """
    ...
    async def check_stale_agents(self) -> List[str]:
        ...
        for agent_id, agent_info in list(self.active_agents.items()):
            ...
            if time_since_heartbeat > timedelta(minutes=5):
                ...
                await update_instance_status(
                    agent_info["container_id"], InstanceStatus.FAILED
                )
                del self.active_agents[agent_id]

                logger.warning(
                    f"Agent {agent_id} marked as lost (no heartbeat for 5+ minutes)"
                )
                ...
                # Update database status
                try:
                    instance = await get_instance_by_container_id(
                        agent_info["container_id"]
                    )
                ...
                logger.warning(
                    f"Agent {agent_id} marked as stale (no heartbeat for 90+ seconds)"
                )
    ...
    def get_healthy_agents(self) -> List[str]:
        ...
        for agent_id, info in self.active_agents.items():
            ...
            if time_since_heartbeat < timedelta(seconds=60) and info[
                "current_status"
            ] in ["ready", "busy"]:
