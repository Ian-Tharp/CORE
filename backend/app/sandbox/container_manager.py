...
class ContainerManager:
    ...
    async def get_container(
        self, config: ContainerConfig, reuse: bool = True
    ) -> ContainerInfo:
        """
    ...
    async def release_container(self, container_id: str, return_to_pool: bool = True):
        ...
        async with self._lock:
            if container_id not in self._active_containers:
                logger.warning(
                    f"Attempted to release unknown container: {container_id}"
                )
                return
    ...
    async def execute_python(
        self, container_id: str, code: str, timeout: Optional[int] = None
    ) -> ExecutionResult:
        """
    ...
    async def _create_container(self, config: ContainerConfig) -> ContainerInfo:
        ...
        try:
            ...
            logger.info(
                f"Created container {container.id} with trust level {security.trust_level}"
            )
            return container_info
    ...
    async def _cleanup_expired_containers(self):
        ...
        async with self._lock:
            ...
            for trust_level in TrustLevel:
                ...
                expired = [
                    c for c in pool if now - c.last_activity > self.container_ttl
                ]
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "pools": {level.value: len(self._pools[level]) for level in TrustLevel},
