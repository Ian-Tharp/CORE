...
async def submit_task(
    request: CreateTaskRequest, api_key: str = Depends(require_api_key)
) -> Dict[str, str]:
    """
...
async def list_tasks_endpoint(
    ...
    assigned_agent_id: Optional[UUID] = Query(
        None, description="Filter by assigned agent"
    ),
    priority_min: Optional[int] = Query(
        None, ge=1, le=10, description="Minimum priority"
    ),
    priority_max: Optional[int] = Query(
        None, ge=1, le=10, description="Maximum priority"
    ),
    human_override: Optional[bool] = Query(
        None, description="Filter by human override"
    ),
    ...
    try:
        ...
        return TaskListResponse(
            tasks=tasks, total_count=total_count, page=page, page_size=page_size
        )
...
async def assign_task_manually(
    task_id: UUID, request: AssignTaskRequest, api_key: str = Depends(require_api_key)
) -> Dict[str, str]:
    """
    ...
    try:
        ...
        success = await update_task_status(
            task_id, TaskStatus.ASSIGNED, assigned_agent_id=request.agent_id
        )
        ...
        logger.info(
            f"Task {task_id} manually assigned to agent {request.agent_id}: {request.reason}"
        )
...
async def fail_task(
    task_id: UUID, request: TaskFailureRequest, api_key: str = Depends(require_api_key)
) -> Dict[str, str]:
...
async def retry_task(
    ...
    try:
        ...
        try:
            ...
            if assignment:
                logger.info(
                    f"Retried task {task_id} routed to agent {assignment.agent_id}"
                )
            else:
