...
class StoreKnowledgeRequest(BaseModel):
    ...
    source_agent_id: Optional[str] = Field(
        None, description="ID of agent storing the knowledge"
    )
...
class StoreExperienceRequest(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Experience metadata"
    )
...
class StoreProcedureRequest(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Procedure metadata"
    )
...
class UpdateProcedureOutcomeRequest(BaseModel):
    ...
    success: bool = Field(
        ..., description="Whether the procedure execution was successful"
    )
...
async def search_knowledge(
    ...
    try:
        ...
        return MemorySearchResponse(
            query=q, results=results, total=len(results), threshold=threshold
        )
...
async def get_agent_experiences(
    ...
    try:
        memories = await service.get_agent_experiences(
            agent_id=agent_id, query=q, limit=limit, memory_type=memory_type
        )
...
async def search_procedures(
    ...
    try:
        procedures = await service.search_procedures(query=q, role=role)
...
async def update_procedure_outcome(
    ...
    try:
        success = await service.update_procedure_outcome(
            procedure_id=request.procedure_id, success=request.success
        )
...
async def get_relevant_context(
    ...
    agent_id: Optional[str] = Query(
        None, description="Agent ID for personalized context"
    ),
...
async def clear_agent_memories(
    ...
    tier: Optional[str] = Query(
        None, description="Memory tier to clear (semantic/episodic/procedural)"
    ),
    service=Depends(get_memory_service),
    api_key: str = Depends(require_api_key),
) -> Dict[str, Any]:
...
async def memory_health_check(
    ...
    try:
        ...
        overall_healthy = (
            ...
            and health.get("embedding_service", False)
            and health.get("database", False)
        )
