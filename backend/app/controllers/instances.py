...
class SpawnInstanceRequest(BaseModel):
    ...
    agent_role: str = Field(
        ..., description="Role of the agent (e.g., 'reasoning', 'comprehension')"
    )
    device_id: Optional[str] = Field(
        None, description="Device ID where instance should run"
    )
...
async def spawn_instance(
    request: SpawnInstanceRequest, api_key: str = Depends(require_api_key)
) -> InstanceResponse:
    """
    ...
    try:
        ...
        logger.info(
            f"Spawned instance: {instance_info.container_id[:12]} for role {request.agent_role}"
        )
...
async def list_instances_endpoint(
    ...
    include_docker: bool = Query(
        True, description="Include live Docker container data"
    ),
    ...
    try:
        if include_docker:
            ...
            if agent_role:
                filtered_instances = [
                    i for i in filtered_instances if i.agent_role == agent_role
                ]

            if status_filter:
                filtered_instances = [
                    i for i in filtered_instances if i.status == status_filter
                ]
            ...
            response_instances = [
                _convert_instance_info(info) for info in paginated_instances
            ]
        else:
...
@router.get(
    "/{instance_id}", status_code=status.HTTP_200_OK, response_model=InstanceResponse
)
async def get_instance_details(
    instance_id: str, api_key: str = Depends(require_api_key)
) -> InstanceResponse:
    """
...
async def scale_instances(
    request: ScaleRequest, api_key: str = Depends(require_api_key)
) -> ScaleResponse:
    """
    ...
    try:
        ...
        result = await instance_manager.scale_instances(
            request.role, request.target_count
        )
...
async def get_instance_trust_metrics(
    instance_id: str, api_key: str = Depends(require_api_key)
    ...
    try:
        ...
        trust_score = (
            (trust_metrics.tasks_completed + 1.0) / (total_tasks + 1.0)
            if total_tasks >= 0
            else 0.5
        )
...
async def get_trust_analytics(
    agent_role: Optional[str] = Query(None, description="Filter by agent role"),
    min_trust_score: Optional[float] = Query(
        None, ge=0.0, le=1.0, description="Minimum trust score"
    ),
    api_key: str = Depends(require_api_key),
    ...
    try:
        results = await get_instances_with_trust_metrics(
            agent_role=agent_role, min_trust_score=min_trust_score
        )
        ...
        for item in results:
            ...
            response_data.append(
                InstanceWithTrustResponse(
                    instance=instance_response, trust_metrics=trust_response
                )
            )
...
async def register_agent_rest(
    request: AgentRegistrationRequest, api_key: str = Depends(require_api_key)
) -> AgentConfigResponse:
    """
    ...
    try:
        ...
        logger.info(
            f"Agent registered via REST: {config.agent_id} (container: {request.container_id[:12]})"
        )
