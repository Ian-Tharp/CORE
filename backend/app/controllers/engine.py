...
NODE_MESSAGES = {
    ...
    "orchestration": {"start": "📋 Planning approach...", "complete": "✓ Plan ready"},
    "reasoning": {"start": "⚡ Executing...", "complete": "✓ Done"},
    "evaluation": {"start": "🔎 Checking quality...", "complete": "✓ Looks good"},
    "conversation": {
...
async def run_core(
    request: RunRequest, api_key: dict = Depends(require_role(Role.AGENT))
) -> RunResponse:
    """
    ...
    try:
        ...
        try:
            await webhook_service.fire(
                ...
                payload={
                    ...
                    "intent_type": (
                        final_state.intent.type if final_state.intent else None
                    ),
        ...
        try:
            await webhook_service.fire(
                ...
                payload={"error": str(e), "errors": state.errors},
                run_id=state.run_id,
            )
...
async def run_core_stream(
    ...
    async def stream_generator():
        ...
        try:
            ...
            yield _sse_event(
                {
                    ...
                    "quality": (
                        state.eval_result.quality_score if state.eval_result else None
                    ),
                    "message": "✅ Complete",
                }
            )
            ...
            yield _sse_event(
                {"event": "error", "error": str(e), "message": f"❌ Error: {str(e)}"}
            )
...
async def stream_run(
    run_id: str, user_input: str = "", api_key: str = Depends(require_api_key)
):
    """
    ...
    async def event_generator():
        ...
        try:
            ...
            logger.info(
                f"[CORE STREAM] Sending final complete event, response: {(final_state.response or 'NONE')[:200]}"
            )
...
async def _stream_graph_execution(graph, state: COREState):
    ...
    try:
        ...
        if state.intent and state.intent.type in ["task", "question"]:
            ...
            logger.info(
                f"[CORE STREAM] Reasoning complete, step_results count: {len(state.step_results)}"
            )

            if state.step_results:
                for result in state.step_results:
                    logger.info(
                        f"[CORE STREAM] Yielding step_executed for {result.step_id}, status={result.status}"
                    )
                    yield {
                        "event": "step_executed",
                        "step_id": result.step_id,
        ...
        logger.info(
            f"[CORE STREAM] Conversation complete, response length: {len(state.response or '')}"
        )
...
async def delete_run(
    run_id: str, api_key: str = Depends(require_api_key)
) -> Dict[str, str]:
    """
    ...
    try:
        ...
        async with pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM core_runs WHERE run_id = $1", run_id
            )
    ...
    if include_db:
        try:
            db_runs = await run_repository.list_runs(
                user_id=user_id, status=status_filter, limit=limit
            )
            for db_run in db_runs:
                ...
                if rid not in runs:  # Don't duplicate in-memory runs
                    runs[rid] = {
                        ...
                        "current_node": db_run.get("current_node"),
                        "user_input": (
                            (db_run.get("user_input", "")[:100] + "...")
                            if db_run.get("user_input")
                            and len(db_run.get("user_input", "")) > 100
                            else db_run.get("user_input", "")
                        ),
                        "started_at": (
                            db_run["created_at"].isoformat()
                            if db_run.get("created_at")
                            else None
                        ),
                        "completed_at": (
                            db_run["completed_at"].isoformat()
                            if db_run.get("completed_at")
                            else None
                        ),
                        "source": "database",
                    }
