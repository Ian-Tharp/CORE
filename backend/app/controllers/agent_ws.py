...
class AgentWebSocketManager:
    ...
    async def _handle_task_refusal(
        self, agent_id: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Handle task refusal."""
        try:
    ...
    async def _handle_deregistration(
        self, agent_id: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            if agent_id in agent_registry.active_agents:
                deregistration = AgentDeregistration(
                    reason="websocket_disconnect", final_state={}
                )
                await agent_registry.deregister_agent(agent_id, deregistration)
        except Exception as e:
...
async def send_task_to_agent(agent_id: str, task_assignment: dict) -> bool:
    ...
    Returns:
        True if sent successfully, False otherwise
    """
    message = {"type": "task_assigned", **task_assignment}
...
async def send_shutdown_request(
    agent_id: str, reason: str, grace_period_seconds: int = 30
) -> bool:
    """
