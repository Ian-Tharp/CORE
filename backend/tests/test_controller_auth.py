...
class TestCreativeAuth:
    ...
    def test_create_wiki_missing_key_returns_401(self):
        """
        SENTINEL — POST /creative/wiki must require auth.
        Remove require_api_key → any caller can create wiki pages → test fails.
        """
        resp = self._client().post(
            "/creative/wiki",
    ...
    def test_list_wiki_missing_key_returns_401(self):
        """GET /creative/wiki must require auth."""
        resp = self._client().get("/creative/wiki")
        assert resp.status_code == 401

    def test_update_wiki_missing_key_returns_401(self):
        """PUT /creative/wiki/{id} must require auth."""
        resp = self._client().put(
            "/creative/wiki/page-1",
            json={"title": "T", "content": "C"},
        )
        assert resp.status_code == 401

    def test_create_character_missing_key_returns_401(self):
        """POST /creative/characters must require auth."""
        resp = self._client().post(
            "/creative/characters",
            json={"name": "Hero", "traits": {}},
        )
        assert resp.status_code == 401

    def test_generate_image_missing_key_returns_401(self):
        """POST /creative/characters/{id}/image must require auth."""
        resp = self._client().post(
            "/creative/characters/char-1/image",
            json={"prompt": "a hero"},
        )
        assert resp.status_code == 401

    def test_create_wiki_valid_key_proceeds(self, api_key):
        """
        SENTINEL — valid key must pass auth on POST /creative/wiki.
        """
        with patch("app.controllers.creative.repo") as mock_repo:
            mock_repo.create_wiki_page = AsyncMock(return_value="page-123")
            resp = self._client().post(
                "/creative/wiki",
                json={"title": "Test", "content": "body"},
                headers={"X-API-Key": api_key},
