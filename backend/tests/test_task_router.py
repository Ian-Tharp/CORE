...
class TestTaskRouting:
    ...
    @pytest.mark.asyncio
    async def test_routes_to_capable_agent_with_lowest_load(
        self, task_router, sample_task, sample_agents_with_metrics
    ):
        """Test that tasks are routed to agents with required capabilities and lowest load."""
        # Arrange
        with patch(
            "app.services.task_router.get_instances_with_trust_metrics"
        ) as mock_get_instances, patch(
            "app.services.task_router.agent_registry"
        ) as mock_registry, patch(
            "app.services.task_router.update_task_status"
        ) as mock_update_status, patch(
            "app.services.task_router.create_task_assignment"
        ) as mock_create_assignment, patch(
            "app.services.task_router.get_agent_task_metrics"
        ) as mock_agent_metrics, patch.object(
            task_router, "_get_agent_current_load"
        ) as mock_get_load:
    ...
    @pytest.mark.asyncio
    async def test_filters_out_agents_missing_required_capabilities(
        self, task_router, sample_agents_with_metrics
    ):
        ...
        task_with_specific_requirements = Task(
            ...
            required_capabilities=[
                "code_generation",
                "debugging",
            ],  # None of sample agents have these
        )
        ...
        with patch(
            ...
            mock_update_status.assert_called_once_with(
                task_with_specific_requirements.id, TaskStatus.QUEUED
            )

    @pytest.mark.asyncio
    async def test_respects_priority_ordering(
        self, task_router, sample_task, sample_agents_with_metrics
    ):
    ...
    @pytest.mark.asyncio
    async def test_handles_no_available_agents_queues_task(
        self, task_router, sample_task
    ):
        ...
        with patch(
            ...
            mock_update_status.assert_called_once_with(
                sample_task.id, TaskStatus.QUEUED
            )

    @pytest.mark.asyncio
    async def test_trust_score_affects_routing_preference(
        self, task_router, sample_task, sample_agents_with_metrics
    ):
...
class TestTaskRefusal:
    ...
    async def test_refusal_triggers_reroute_to_next_best_agent(self, task_router):
        ...
        with patch(
            ...
            "app.services.task_router.increment_task_refused"
        ) as mock_increment_refused, patch(
            "app.services.task_router.update_task_status"
        ) as mock_update_status:
            ...
            mock_update_status.assert_called_once_with(
                task_id, TaskStatus.FAILED, result={"error": "Agent refused task"}
            )
    ...
    async def test_suggested_agent_gets_priority_on_reroute(self, task_router):
        ...
        with patch(
            ...
            # Act
            result = await task_router.handle_task_refusal(
                task_id, agent_id, reason, suggested_agent
            )
...
class TestTaskCompletion:
    ...
    async def test_successful_completion_updates_metrics(self, task_router):
        ...
        with patch(
            ...
            "app.services.task_router.create_task_result"
        ) as mock_create_result, patch(
            "app.services.task_router.get_instances_with_trust_metrics"
        ) as mock_get_instances, patch(
            "app.services.task_router.increment_task_completed"
        ) as mock_increment_completed:
    ...
    async def test_failed_task_can_be_retried(self, task_router):
        ...
        with patch(
            ...
            "app.services.task_router.increment_task_failed"
        ) as mock_increment_failed:
            ...
            await task_router.handle_task_failure(
                task_id, error, agent_id, duration_ms, model_used
            )
...
class TestAnalytics:
    ...
    async def test_per_agent_task_metrics_calculated_correctly(self, task_router):
        ...
        # Arrange
        with patch(
            "app.services.task_router.get_task_metrics"
        ) as mock_get_task_metrics, patch(
            "app.services.task_router.get_agent_task_metrics"
        ) as mock_get_agent_metrics:
...
class TestEdgeCases:
    ...
    async def test_task_with_impossible_capabilities(self, task_router):
        ...
        with patch(
            ...
            assert result is None
            mock_update_status.assert_called_once_with(
                impossible_task.id, TaskStatus.QUEUED
            )

    @pytest.mark.asyncio
    async def test_all_agents_at_max_capacity(
        self, task_router, sample_task, sample_agents_with_metrics
    ):
    ...
    async def test_agent_goes_offline_during_task_execution(self, task_router):
        ...
        with patch(
            ...
            "app.services.task_router.increment_task_failed"
        ) as mock_increment_failed, patch(
            "app.services.task_router.update_task_status"
        ) as mock_update_status, patch(
            "app.services.task_router.create_task_result"
...
class TestHelperFunctions:
    ...
    def test_capability_match_score_calculation(self, task_router):
        ...
        agent_extra = MagicMock(
            capabilities=["web_search", "analysis", "writing", "coding"]
        )
        ...
        score_missing = task_router._calculate_capability_match_score(
            task, agent_missing
        )
    ...
    def test_model_preference_score_calculation(self, task_router):
        ...
        task_with_preference = Task(
            id=uuid4(), task_type=TaskType.RESEARCH, preferred_model="claude-sonnet"
        )
        task_no_preference = Task(id=uuid4(), task_type=TaskType.ANALYSIS)

        researcher_agent = MagicMock(agent_role="researcher")
        coder_agent = MagicMock(agent_role="coder")

        # Act
        score_preferred = task_router._calculate_model_preference_score(
            task_with_preference, researcher_agent
        )
        score_no_preference = task_router._calculate_model_preference_score(
            task_no_preference, researcher_agent
        )
        score_mismatch = task_router._calculate_model_preference_score(
            task_with_preference, coder_agent
        )

        # Assert
        assert (
            score_preferred == 1.0
        )  # Researcher role matches claude-sonnet preference
