...
class TestAgentRegistration:
    ...
    async def test_register_agent_success(
        self, agent_registry, mock_instance, registration_payload
    ):
        ...
        with patch(
            ...
            with patch(
                "app.services.agent_registry.update_instance", return_value=True
            ):
    ...
    @pytest.mark.asyncio
    async def test_register_agent_unknown_container(
        self, agent_registry, registration_payload
    ):
    ...
    @pytest.mark.asyncio
    async def test_duplicate_registration_updates_record(
        self, agent_registry, mock_instance, registration_payload
    ):
...
class TestHeartbeat:
    ...
    async def test_heartbeat_updates_timestamp_and_status(
        self, agent_registry, mock_instance, registration_payload, heartbeat_data
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "app.services.agent_registry.update_heartbeat", return_value=True
                ):
                    # Register agent first
                    config = await agent_registry.register_agent(registration_payload)
                    agent_id = config.agent_id
                    initial_heartbeat = agent_registry.active_agents[agent_id][
                        "last_heartbeat"
                    ]
                    ...
                    # Act
                    response = await agent_registry.handle_heartbeat(
                        agent_id, heartbeat_data
                    )
    ...
    @pytest.mark.asyncio
    async def test_heartbeat_with_resource_usage_stores_metrics(
        self, agent_registry, mock_instance, registration_payload
    ):
    ...
    @pytest.mark.asyncio
    async def test_heartbeat_from_unregistered_agent_rejected(
        self, agent_registry, heartbeat_data
    ):
...
class TestDeregistration:
    ...
    @pytest.mark.asyncio
    async def test_graceful_deregister_saves_final_state(
        self, agent_registry, mock_instance, registration_payload
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "app.services.agent_registry.update_instance_status",
                    return_value=True,
                ):
                    config = await agent_registry.register_agent(registration_payload)
                    agent_id = config.agent_id
    ...
    @pytest.mark.asyncio
    async def test_deregister_updates_status_to_stopped(
        self, agent_registry, mock_instance, registration_payload
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    deregistration = AgentDeregistration(
                        reason="container_stop", final_state={}
                    )
...
class TestStaleAgentDetection:
    ...
    @pytest.mark.asyncio
    async def test_agent_with_old_heartbeat_marked_unhealthy(
        self, agent_registry, mock_instance, registration_payload
    ):
    ...
    @pytest.mark.asyncio
    async def test_agent_missing_5_minutes_marked_as_lost(
        self, agent_registry, mock_instance, registration_payload
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        "app.services.instance_manager.instance_manager.restart_instance",
                        return_value=True,
                    ) as mock_restart:
                        config = await agent_registry.register_agent(
                        ...
                        agent_registry.active_agents[agent_id][
                            "last_heartbeat"
                        ] = old_time
    ...
    @pytest.mark.asyncio
    async def test_healthy_agent_not_affected_by_stale_check(
        self, agent_registry, mock_instance, registration_payload
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                agent_registry.active_agents[agent_id]["last_heartbeat"] = datetime.now(
                    timezone.utc
                )
...
class TestTaskRefusal:
    ...
    @pytest.mark.asyncio
    async def test_agent_can_refuse_task_with_reason(
        self, agent_registry, mock_instance, registration_payload
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "app.services.agent_registry.increment_task_refused",
                    return_value=True,
                ) as mock_increment:
                    config = await agent_registry.register_agent(registration_payload)
                    agent_id = config.agent_id
    ...
    @pytest.mark.asyncio
    async def test_refused_task_recorded_in_trust_metrics(
        self, agent_registry, mock_instance, registration_payload
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    refusal = TaskRefusal(
                        task_id="task_789", reason="Too complex", suggested_agent=None
                    )
...
class TestTaskCompletion:
    ...
    @pytest.mark.asyncio
    async def test_task_completion_updates_trust_metrics(
        self, agent_registry, mock_instance, registration_payload
    ):
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_websocket_disconnects_unexpectedly_agent_marked_unhealthy(
        self, ws_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_multiple_rapid_heartbeats_handled_correctly(
        self, agent_registry, mock_instance, registration_payload
    ):
    ...
    @pytest.mark.asyncio
    async def test_registration_during_shutdown_rejected(
        self, agent_registry, registration_payload
    ):
    ...
    @pytest.mark.asyncio
    async def test_invalid_json_in_websocket_message_handled_gracefully(
        self, ws_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_registrations_from_same_container_id(
        self, agent_registry, mock_instance
    ):
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_lifecycle_spawn_register_heartbeat_task_deregister(
        self, agent_registry, mock_instance, registration_payload
    ):
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            # Act 2: Heartbeat
                            heartbeat = AgentHeartbeatData(
                                status="ready", current_task=None, resource_usage={}
                            )
                            response = await agent_registry.handle_heartbeat(
                                agent_id, heartbeat
                            )
                            ...
                            task = TaskAssignment(
                                task_id="integration_task", task_type="test", payload={}
                            )
                            task_assigned = await agent_registry.assign_task(
                                agent_id, task
                            )

                            completion = TaskCompletion(
                                task_id="integration_task", result={}, duration_ms=1000
                            )
                            await agent_registry.handle_task_completion(
                                agent_id, completion
                            )

                            # Act 4: Deregister
                            deregistration = AgentDeregistration(
                                reason="test_complete", final_state={}
                            )
                            await agent_registry.deregister_agent(
                                agent_id, deregistration
                            )
            ...
            with patch(
                ...
                for i in range(2):
                    ...
                    with patch(
                        ...
                        await agent_registry.deregister_agent(
                            configs[i].agent_id, deregistration
                        )
