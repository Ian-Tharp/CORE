...
class TestSecurityConfig:
    ...
    def test_validate_security_config_untrusted_with_network(self):
        ...
        config = AgentSecurityConfig(
            trust_level=TrustLevel.UNTRUSTED, network_enabled=True  # This is bad
        )
    ...
    def test_validate_security_config_low_memory(self):
        ...
        config = AgentSecurityConfig(memory_limit_mb=32)  # Too low
...
class TestContainerManager:
    ...
    def manager(self):
        ...
        return ContainerManager(pool_size=2, container_ttl_minutes=5)
    ...
    def test_build_docker_config_trusted(self, manager):
        ...
        config = ContainerConfig(
            name="test-container", security=TRUST_PRESETS[TrustLevel.TRUSTED]
        )
    ...
    def test_build_docker_config_untrusted(self, manager):
        ...
        config = ContainerConfig(
            name="test-untrusted", security=TRUST_PRESETS[TrustLevel.UNTRUSTED]
        )
    ...
    def test_build_docker_config_sandboxed(self, manager):
        ...
        config = ContainerConfig(
            name="test-sandboxed", security=TRUST_PRESETS[TrustLevel.SANDBOXED]
        )
...
class TestStateManager:
    ...
    def state_manager(self, tmp_path):
        ...
        return StateManager(data_dir=str(tmp_path / "state_data"), state_ttl_minutes=5)
    ...
    async def test_set_and_get_state(self, state_manager):
        ...
        retrieved = await state_manager.get_state(
            agent_id="test-agent", task_id="test-task"
        )
    ...
    async def test_state_update(self, state_manager):
        ...
        # Set initial state
        await state_manager.set_state(
            agent_id="agent1", task_id="task1", data={"step": 1, "status": "running"}
        )

        # Update state
        await state_manager.set_state(
            agent_id="agent1", task_id="task1", data={"step": 2}  # Only update step
        )
    ...
    async def test_state_deletion(self, state_manager):
        ...
        # Set state
        await state_manager.set_state(
            agent_id="agent1", task_id="task1", data={"key": "value"}
        )
    ...
    async def test_list_artifacts(self, state_manager):
        ...
        await state_manager.save_artifact(
            agent_id="agent1", task_id="task1", name="file1.txt", content=b"Content 1"
        )

        await state_manager.save_artifact(
            agent_id="agent1", task_id="task1", name="file2.txt", content=b"Content 2"
        )
...
class TestSandboxIntegration:
    ...
    async def test_security_config_applied_to_container(self):
        ...
        config = ContainerConfig(name="test-integration", security=security)
