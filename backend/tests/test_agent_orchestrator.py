...
class TestDynamicToolSelector:
    ...
    def selector(self):
        ...
        return DynamicToolSelector(
            registry_url="http://localhost:8000", cache_ttl_minutes=5
        )
    ...
    def test_prioritize_capabilities(self, selector):
        ...
        capabilities = [Capability.WEB_SEARCH.value, Capability.FILE_WRITE.value]
    ...
    def test_check_constraints_blocked_tools(self, selector):
        ...
        result = selector._check_constraints(
            tool, {"blocked_tools": ["dangerous_tool", "other_tool"]}
        )
        assert result == False

        # Should pass constraint
        result = selector._check_constraints(tool, {"blocked_tools": ["other_tool"]})
        assert result == True
    ...
    def test_generate_selection_reason_with_tools(self, selector):
        ...
        reason = selector._generate_selection_reason(
            selected_tools=tools, matched_caps={"web:search"}, missing_caps=[]
        )
...
class TestAgentOrchestratorWithMocks:
    ...
    def mock_container_manager(self):
        ...
        manager.get_container = AsyncMock(
            return_value=Mock(
                id="container-123", name="test-container", status="running"
            )
        )
        manager.release_container = AsyncMock()
        manager.get_status = Mock(return_value={"pools": {}, "active_containers": 0})
        return manager
    ...
    def mock_state_manager(self, tmp_path):
        ...
        manager.initialize = AsyncMock()
        manager.set_state = AsyncMock(return_value=Mock(id="state-123", data={}))
        manager.get_state = AsyncMock(return_value=None)
        manager.get_status = Mock(
            return_value={"state_entries": 0, "subscribed_agents": 0}
        )
        return manager
    ...
    def mock_tool_selector(self):
        ...
        selector.select_tools_for_task = AsyncMock(
        ...
        selector.get_status = Mock(
            return_value={"cached_tools": 10, "indexed_capabilities": 5}
        )
        return selector

    @pytest.mark.asyncio
    async def test_orchestrator_initialization(
        self, mock_container_manager, mock_state_manager, mock_tool_selector
    ):
...
class TestCORELoop:
    ...
    def setup_orchestrator(self, tmp_path):
        ...
        container_manager.get_container = AsyncMock(
            return_value=Mock(
                id="container-123", name="test-container", status="running"
            )
        )
        ...
        tool_selector.select_tools_for_task = AsyncMock(
            return_value=ToolSelection(
                selected_tools=[],
    ...
    async def test_core_loop_simple_query(self, setup_orchestrator):
        ...
        result = await orchestrator.execute_core_loop(
            user_input="Hello, how are you?", session_id="test-session"
        )
    ...
    async def test_core_loop_max_iterations(self, setup_orchestrator):
        ...
        result = await orchestrator.execute_core_loop(
            user_input="Complex task", session_id="test-session", max_iterations=1
        )
